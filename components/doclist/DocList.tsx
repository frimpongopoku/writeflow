...
export function DocList({ collapsed, onToggle }: DocListProps) {
  ...
  return (
    <section
      className={cn(
        "relative flex flex-col h-full bg-background border-r border-border shrink-0 overflow-hidden transition-[width] duration-200",
    ...
    >
      ...
      {collapsed && (
        <button
          onClick={onToggle}
          className="absolute inset-0 flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-hover transition-colors"
      ...
      {/* ── Expanded content ── */}
      <div className={cn(
        "flex flex-col h-full min-w-[260px] transition-opacity duration-150",
